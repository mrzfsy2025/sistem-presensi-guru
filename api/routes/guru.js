...
router.get('/status', isGuru, async (req, res) => {
    ...
    try {
        ...
        const [profilRows] = await db.query(
            "SELECT nama_lengkap, nip_nipppk, email FROM guru WHERE id_guru = $1;", // Tambahkan email
            [id_guru]
        );
        ...
        const [presensiRows] = await db.query(
            "SELECT jam_masuk, jam_pulang FROM presensi WHERE id_guru = $1 AND tanggal = $2;",
...
router.put('/profile/password', isGuru, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.query("SELECT password_hash FROM guru WHERE id_guru = $1;", [id_guru]);
        ...
        await db.query("UPDATE guru SET password_hash = $1 WHERE id_guru = $2;", [password_hash_baru, id_guru]);
